...
* I read the ‘Warning’ dialog and the click the `YES` button:
...
* I wait for the bootloader to rewrite the EEPROM.
    
* A successful rewrite is shown by a blinking, green LED and a green screen displayed on the monitor.
    
* I repeat this process for the remaining Pi5 SBCs.
    

> NOTE: If there is no bootable micro SD card in the Pi 5, then it will try to boot from the M.2 NVMe drive I installed earlier.
