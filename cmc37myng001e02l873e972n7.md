...
## Creating a Micro SD Card Bootloader.
...
* I insert the micro SD card into the Pi5 micro SD card slot.
    
* I power on the Pi5.
    
* I wait for the micro SD card to rewrite the bootloader in the EEPROM of the Pi5.
...
> NOTE: If there is no bootable micro SD card in the Pi 5, then it will try to boot from an M.2 NVMe drive. If there is no bootable M.2 NVMe drive, the it will try to boot from a USB drive.
