...
There is a redundancy requirement when assembling an HA (High Availability) cluster. Many different flavours of Kubernetes (K8s, K3s, MicroK8s, K0s, and even Minikube) support HA clustering and I will use my Homelab hardware, such as it is, to practice the skills of deploying HA solutions. Using Pi5 SBCs as HA control planes is fine due to the low resource requirements. Worker nodes (sometimes called data planes), on the other hand, consumes many resources (CPU cycles, RAM, bandwidth, etc.) due to processing API requests. Not only do worker nodes need to send a LOT of data, but they must also spin up more nodes when the demand increases. More nodes means more resources consumed.

For M.2 drives, there is a speed difference between the (older) SATA interface and the (newer) NVMe interface. The drives I use in this post are [NVMe PCIe Gen3 x4](https://www.transcend-info.com/product/internal-ssd/mte400s) ***↗,*** as is the enclosure I use and the NVMe hats that are installed in the Pi5 SBCs. Having said that, I also have a SATA M.2 enclosure but I am having trouble locating an M.2 2242 SATA drive to confirm this process works on older technologies. Comment below if you have tried using SATA M.2 drives. Enquiring minds will be eternally grateful.

![](https://cdn.hashnode.com/res/hashnode/image/upload/v1750298647369/f9bca2be-1ddf-4760-937d-8121403d925d.jpeg align="center")

For now, my focus is on installing a Linux distro on M.2 drives that will be used by a Pi5 SBCs.
...
* Connections to an Internet router or a LAN switch (managed or unmanaged).
...
> NOTE: Public key encryption for SSH will be set at a later time.
...
## Assembling a Pi5 the Supports an M.2 Drive.
...
* I connect the NVMe hat to the Pi5 header and use the included large screw to attach it to the active cooler fan.
    
* I use the NVMe ribbon cable to connect the NVMe hat to the Pi5.
    
* I install the M.2 drive, the drive with Ubuntu Server 24.4 LTS installed, into the NVMe hat and use the included small screw to secure the drive.
...
My plan is to use PiLab1 as the primary control plane, PiLab2 as the secondary control plane, and PiLab3 as the principle data store (as well as the host for Pi-hole). PiLab3 will also use my NAS as a redundant back up for the data.

Implementation details will follow in a later post.
